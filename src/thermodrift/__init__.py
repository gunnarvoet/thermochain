...
Each thermistor's internal clock is anchored to UTC during the raw → L0 conversion. Two clock-cal CTD-cast times are stored per sensor in the sensor spreadsheet (`time_cal1`, `time_cal2`) and surfaced through `thermodrift.io.ProcessThermistorMooring.get_clock_cals`. For RBR Solo these are passed to `rbrmoored.solo.proc` as the `cal_time` argument with `apply_time_offset=True`, which corrects the linear drift of the rsk-internal clock between the two cast times. For SBE 56, `get_clock_reads` additionally surfaces `clock_read_utc` / `clock_read_logger` — the logger and reference clock readouts taken on deck — which are forwarded to `sbemoored.sbe56.proc` to bracket the clock offset. The dispatch happens through `run_proc_single_rbr` / `run_proc_single_sbe`; one L0 NetCDF per sensor is written to the configured `procl0` directory.

# CTD rosette calibration

Each sensor is calibrated against co-located CTD rosette casts at deployment and recovery. Pre- and post-deployment offset arrays (one offset per serial number, computed externally and stored as NetCDF) are consumed by `thermodrift.io.rbr_cut_and_cal_interp` during the L0 → L1 step. When both endpoints are available, `thermodrift.io.apply_interpolated_ctd_offset` applies a **linear-in-time interpolation** between them; the interpolation factor is clamped to `[0, 1]` so samples outside the cal bracket receive the corresponding endpoint offset. When only one endpoint is available, the function falls back to a scalar offset via `rbr_apply_ctd_offset`. A `cal_method` attribute is written to each L1 file recording which path was taken (`linear_interp`, `scalar`, or `none`).
...
Implemented in `thermodrift.io.sensor_drift`. With `run_all=True` the class runs the following stages in sequence; each is also callable individually for debugging.
...
The output is a per-sensor drift trace `drift(sn, time)`; applying it (L2 = L1 − drift on the sensor's native time axis) is performed downstream of `thermodrift`.
